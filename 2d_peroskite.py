...
df1 = pd.read_csv("perovskite.csv", encoding_errors='ignore')
df2 = pd.read_csv("Periodic Table of Elements.csv", encoding_errors='ignore')
df3 = pd.read_csv('perovskite.csv', encoding_errors='ignore')
df1_copy = df1
df2_copy = df2
df2_copy.index[1]
...
df1_copy.filter(regex='Number').astype(np.int8)  # 把所有含number的列数据类型设置为整数

# # 想要真正的改变数据框，通常需要通过赋值来进行，比如
# df["Customer Number"] = df["Customer Number"].astype("int")
df1_copy[df1_copy.filter(regex='Number').columns] = df1_copy.filter(regex='Number').astype(np.int8)
# df1_copy.Ion=df3.Ion

df1_copy.to_csv('final_data.csv')
